  ...
  useEffect(() => {
    ...
    if (lastEntityUpdate) {
      ...
      if (entityType === 'SUPPLIER' || entityType === 'PRODUCT') {
        console.log('SupplierList: Refetching data due to entity update:', lastEntityUpdate);
        fetchSuppliers();
      }
    }
  }, [entityUpdateTrigger]);
      ...
      <Box sx={{ p: { xs: 2, sm: 3 }, backgroundColor: '#f8f9fa', minHeight: '100vh' }}>
        ...
        <Paper elevation={2} sx={{ p: { xs: 1.5, sm: 2 }, mb: 3, borderRadius: '12px' }}>
          <TextField
            fullWidth
            variant="outlined"
            placeholder={isMobile ? "Search suppliers..." : "Search suppliers by name, contact, email, phone or address"}
            value={searchTerm}
