...
const ModernCategoryList = () => {
  ...
  useEffect(() => {
    ...
    if (lastEntityUpdate) {
      ...
      if (entityType === 'CATEGORY' || entityType === 'PRODUCT') {
        console.log('CategoryList: Refetching data due to entity update:', lastEntityUpdate);
        fetchCategories();
      }
    }
  }, [entityUpdateTrigger]);
      ...
      <Box sx={{ p: { xs: 2, sm: 3 }, backgroundColor: '#f8f9fa', minHeight: '100vh' }}>
        ...
        <Paper elevation={2} sx={{ p: { xs: 1.5, sm: 2 }, mb: 3, borderRadius: '12px' }}>
          <TextField
            fullWidth
            variant="outlined"
            placeholder={isMobile ? "Search categories..." : "Search categories by name or description"}
            value={searchTerm}
        ...
        >
          <DialogTitle sx={{ pb: 1 }}>
            <Typography variant="h5" component="span" fontWeight={600} color="#3a7bd5">
