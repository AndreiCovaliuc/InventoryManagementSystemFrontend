...
const ModernProductList = () => {
  ...
  const { entityUpdateTrigger, lastEntityUpdate } = usePresence();
  ...
  // Listen for real-time entity updates via WebSocket
  useEffect(() => {
    ...
    // Refetch data when relevant entities are updated
    if (lastEntityUpdate) {
      const { entityType } = lastEntityUpdate;
      if (entityType === 'PRODUCT' || entityType === 'CATEGORY' || entityType === 'SUPPLIER') {
        console.log('ProductList: Refetching data due to entity update:', lastEntityUpdate);
        fetchData();
      }
    }
  }, [entityUpdateTrigger]);
          ...
          <Grid container spacing={2} alignItems="center">
            ...
            <Grid item xs={6} md={3}>
              <FormControl fullWidth variant="outlined" size={isMobile ? "small" : "medium"}>
                <InputLabel id="category-filter-label">{isMobile ? "Category" : "Filter by Category"}</InputLabel>
                <Select
                  labelId="category-filter-label"
                  name="categoryId"
                  value={filters.categoryId}
                  onChange={handleFilterChange}
                  label={isMobile ? "Category" : "Filter by Category"}
                  sx={{
                    borderRadius: '8px',
            ...
            <Grid item xs={6} md={3}>
              <FormControl fullWidth variant="outlined" size={isMobile ? "small" : "medium"}>
                <InputLabel id="supplier-filter-label">{isMobile ? "Supplier" : "Filter by Supplier"}</InputLabel>
                <Select
                  labelId="supplier-filter-label"
                  name="supplierId"
                  value={filters.supplierId}
                  onChange={handleFilterChange}
                  label={isMobile ? "Supplier" : "Filter by Supplier"}
                  sx={{
                    borderRadius: '8px',
                    '& .MuiOutlinedInput-notchedOutline': {
                      borderColor: '#ddd',
                      ...
                      <CardContent sx={{ p: 2 }}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', flex: 1 }}>
                            ...
                            >
                              <ProductIcon sx={{ color: '#3a7bd5', fontSize: '1.2rem' }} />
                            </Box>
                            <Box sx={{ flex: 1, minWidth: 0 }}>
                              ...
                              <Chip
                                ...
                                size="small"
                                variant="outlined"
                                sx={{ borderColor: '#aaa', fontFamily: 'monospace', fontSize: '0.7rem', height: 20 }}
                              />
                            </Box>
                          </Box>
                        ...
                        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mt: 1 }}>
                          ...
                          {product.supplierName && (
                            <Chip
                              icon={<SupplierIcon sx={{ fontSize: '0.9rem !important' }} />}
